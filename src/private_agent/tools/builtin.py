...
def _python_command() -> str:
    return "python.exe" if _is_windows() else "python3"


def _gradle_wrapper_command(repo_root: Path) -> str:
    windows_wrapper = repo_root / "gradlew.bat"
    unix_wrapper = repo_root / "gradlew"
    if _is_windows() and windows_wrapper.exists():
        return str(windows_wrapper)
    if unix_wrapper.exists():
        return str(unix_wrapper)
    if windows_wrapper.exists():
        return str(windows_wrapper)
    return str(unix_wrapper if not _is_windows() else windows_wrapper)
...
def run_repo_command(data: RunRepoCommandInput, context: ToolContext) -> dict[str, Any]:
    ...
    allowed_commands: dict[str, tuple[list[str], int]] = {
        ...
        "pytest": ([_python_command(), "-m", "pytest", "-q"], 300),
        "gradle_test": ([_gradle_wrapper_command(repo_root), "test"], 600),
    }
