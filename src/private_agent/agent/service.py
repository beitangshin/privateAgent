...
class AgentService:
    ...
    def create_inventory_storage(self, message: IncomingMessage, storage_name: str) -> HandleResult:
        ...
        if self._inventory_store is None:
            return HandleResult(trace_id=trace_id, status="error", message="inventory sync is not configured")
        result = self._inventory_store.create_storage(storage_name)
        return HandleResult(trace_id=trace_id, status="ok", message=result["message"], data=result)

    def create_inventory_box(
        self, message: IncomingMessage, storage_name: str, box_name: str
    ) -> HandleResult:
        trace_id = uuid.uuid4().hex[:12]
        self._authorizer.verify(message.sender_id)
        self._authorizer.verify_chat(message.chat_id)
        if self._inventory_store is None:
            return HandleResult(trace_id=trace_id, status="error", message="inventory sync is not configured")
        result = self._inventory_store.create_box(storage_name, box_name)
        return HandleResult(trace_id=trace_id, status="ok", message=result["message"], data=result)
    ...
    def upsert_inventory_item(
        ...
        *,
        ...
        if self._inventory_store is None:
            return HandleResult(trace_id=trace_id, status="error", message="inventory sync is not configured")
        result = self._inventory_store.upsert_item(
    ...
    def move_inventory_item(
        self, message: IncomingMessage, *, storage_name: str, item_name: str, target_box_name: str
    ) -> HandleResult:
        trace_id = uuid.uuid4().hex[:12]
        self._authorizer.verify(message.sender_id)
        self._authorizer.verify_chat(message.chat_id)
        if self._inventory_store is None:
            return HandleResult(trace_id=trace_id, status="error", message="inventory sync is not configured")
        result = self._inventory_store.move_item(
            storage_name=storage_name,
            item_name=item_name,
    ...
    def delete_inventory_item(
        self, message: IncomingMessage, *, storage_name: str, item_name: str
    ) -> HandleResult:
        trace_id = uuid.uuid4().hex[:12]
        self._authorizer.verify(message.sender_id)
        self._authorizer.verify_chat(message.chat_id)
        if self._inventory_store is None:
            return HandleResult(trace_id=trace_id, status="error", message="inventory sync is not configured")
        result = self._inventory_store.delete_item(storage_name=storage_name, item_name=item_name)
        return HandleResult(trace_id=trace_id, status="ok", message=result["message"], data=result)
